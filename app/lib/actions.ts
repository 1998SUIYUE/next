...
export async function createInvoice(prevState: State, formData: FormData) {
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/debts`, {
      method: "POST",
      headers: {
...
export async function deleteInvoice(id: string) {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/debts/${id}`, {
      method: "DELETE",
    });
    if (!response.ok) {
...
export async function deleteConsum(id: string) {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/cusum/${id}`, {
      method: "DELETE",
    });
...
export async function createorders(orderData: OrderData) {
  try {
    ...
    // 订单提交成功，增加计数
    const counterResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/order-counter`, { method: "POST" });
